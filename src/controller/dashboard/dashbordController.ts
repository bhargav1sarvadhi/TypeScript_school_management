...
export class DashboardController {
    ...
    async PrincipalDashboard(req, res, next) {
        ...
        res.status(200).json({ success: true, StatusCode: 200, data: { student, teacher,malePercentage,femalePercentage }, message: 'Data  Finded Successfully' });
    }

    async PrincipalAttendance(req, res, next) {
        ...
        return res.status(200).json({ success: true, StatusCode: 200, attendanceData: attendanceData, message: 'Data  Finded Successfully' });
    }
    ...
    async TeacherDashboard(req, res, next) {
        ...
        res.status(200).json({ success: true, StatusCode: 200, data: { scheduleData, profile,holiday }, message: 'Data  Finded Successfully' });
    }
    ...
    async studentDashboard(req, res, next) {
        ...
        const profile = await StudentModel.findOne({ where: { id: req.user.id }});
        const holiday = await holidayModel.findAll({});
        return res.status(200).json({ success: true, StatusCode: 200, data: { profile,schedule,holiday }, message: 'Data Finded Successfully' });
    }
}

// <<<<<<<<<<<<<<<<<<<<<<<<---------------create long  method and using javasciprt logic build------------------>>>>>>>>>> FOR KNOWLEDGE
